...
'''
MELHORIAS VERSAO 2.0: 

> caminhos\path dos arquivos ajustados para ambiente virtual e criação de um app.exe
> adição da tabview
> adição do programa abrir no tamanho padrao maximizado (geometria responsiva)
> exclusão do botao bebidas e adição de adicionar outros produtos e bebidas na hora de registrar um corte
> adição da necessidade de senha para exclusão de alguma row no dtbs
> adição da opção de 2 form de pgmt
'''
...
def Refresh_Widgets(arg):
    try:
        if arg==1:#Registrar Entrada Frame wgs
            profissional_var.set(profissionais[0])
...
def RegistrarEntrada(wb):
    try:
        #testa o sistema de re-write do lastID
        #id
        ...
        if check == True:
            ...
            Refresh_Widgets(1)
            #show check
            Show_success_msgbox(janela_main, 'Entrada registrada com Sucesso!')
        else:
            print('row desaprovada para entrar no database')
    except:
        print('Erro na função RegistrarEntrada, arquivo app.py. Nenhum valor retornado')
...
def RegistrarSaida():
    try:
        ...
        if check == True:
            ...
            Refresh_Widgets(2)
            #show check
            Show_success_msgbox(janela_main, 'Saída registrada com Sucesso!')
        else:
            print('row desaprovada para entrar no database')
    except:
...
def Atualizar():
    try:
        ...
        Show_success_msgbox(janela_main, 'Dados atualizadas com sucesso!')
    except:
...
def DeleteTreeviewData():
    try:
        for i in tv.get_children():
            tv.delete(i)
        print('Treeview limpa com sucesso!')
    ...
    try:
        ...
        for row in rows:
            if str(row[0]) == str(id):
                ...
                break
            index+=1
        Show_success_msgbox(janela_main, 'Item deletado com Sucesso!')
    except:
...
profissional_var = StringVar(value=profissionais[0])
last_selected_profissional_var = StringVar(value=profissionais[0])
...
tb_0_fm_0_wg_2.place(relx=0.25, rely=0.1)
...
tb_0_fm_0_wg_2_1.place(relx=0.50, rely=0.1)
...
tb_0_fm_0_wg_2_2.place(relx=0.75, rely=0.1)
